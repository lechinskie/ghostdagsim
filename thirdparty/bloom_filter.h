...
public:
  ...
  // Construct from raw serialized data (for deserialization).
  bloom_filter(unsigned long long int table_size,
               unsigned int salt_count,
               unsigned long long int random_seed,
               const std::vector<unsigned char>& bit_table)
  ...
  inline unsigned long long int random_seed() const { return random_seed_; }

  inline const std::vector<bloom_type>& salt() const { return salt_; }

  inline const std::vector<unsigned char>& bit_table() const {
    return bit_table_;
  }

  double desired_fpp() const { return desired_false_positive_probability_; }
  ...
  inline std::size_t hash_count() const { return salt_.size(); }
