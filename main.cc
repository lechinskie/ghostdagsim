...
void PrintTotalStats(NodeStats* stats,
                     int totalNodes,
                     double start,
                     double finish,
...
main(int argc, char* argv[])
{
    ...
    double defaultMinersHash[] = {0.15, 0.15, 0.12, 0.12, 0.10, 0.10, 0.08, 0.08, 0.05, 0.05};

    enum Region defaultMinersRegions[] = {NORTH_AMERICA,
                                          EUROPE,
                                          ASIA_PACIFIC,
                                          NORTH_AMERICA,
                                          EUROPE,
    ...
    cmd.AddValue("noBlocks", "Number of blocks to generate", targetNumberOfBlocks);
    cmd.AddValue("nodes", "Total number of nodes", totalNoNodes);
    cmd.AddValue("miners", "Number of miners", noMiners);
    cmd.AddValue("minConnections", "Minimum connections per node", minConnectionsPerNode);
    cmd.AddValue("maxConnections", "Maximum connections per node", maxConnectionsPerNode);
    cmd.AddValue("blockInterval",
                 "Average block generation interval (seconds)",
    ...
    // Create topology
    GhostDagTopologyHelper topologyHelper(systemCount,
                                          totalNoNodes,
                                          noMiners,
                                          minersRegions,
    ...
    for (auto& miner : miners)
    {
        ...
        {
            GhostDagNodeHelper minerHelper(InetSocketAddress(Ipv4Address::GetAny(), ghostdagPort),
                                           nodesConnections[miner],
                                           peersDownloadSpeeds[miner],
                                           peersUploadSpeeds[miner],
                                           nodesInternetSpeeds[miner],
                                           &stats[miner]);
        ...
        if (systemId == targetNode->GetSystemId())
        {
            if (std::find(miners.begin(), miners.end(), node.first) == miners.end())
            {
                GhostDagNodeHelper nodeHelper(
                    ...
                    node.second,
                    peersDownloadSpeeds[node.first],
                    peersUploadSpeeds[node.first],
    ...
    mobility.InstallAll();
    ns3::AnimationInterface::SetConstantPosition(ghostdagNodes.Get(0)->GetNode(), 10.0, 10.0);
    Simulator::Run();
    Simulator::Destroy();
    ...
    MPI_Aint disp[15];
    MPI_Datatype dtypes[15] = {MPI_INT,
                               MPI_DOUBLE,
                               MPI_DOUBLE,
                               MPI_DOUBLE,
                               MPI_INT,
    ...
    else if (systemId == 0 && systemCount > 1)
    {
        ...
        while (count < totalNoNodes)
        {
            MPI_Status status;
            NodeStats recv;
            MPI_Recv(&recv, 1, mpi_nodeStatsType, MPI_ANY_SOURCE, 8888, MPI_COMM_WORLD, &status);
    ...
    if (systemId == 0)
    {
        ...
        std::cout << "Simulated " << stop << " minutes in " << tFinish - tStart << "s\n";
        std::cout << "Performance: " << stop * secsPerMin / (tFinish - tStart) << "x realtime\n";
        std::cout << "Setup time: " << tStartSimulation - tStart << "s\n";
        std::cout << "Network: " << totalNoNodes << " nodes (" << noMiners << " miners)\n";
        std::cout << "Connections: " << minConnectionsPerNode << "-" << maxConnectionsPerNode
                  << " per node\n";
        std::cout << "GHOSTDAG k: " << (int)ghostdagK << "\n";
        std::cout << "Block interval: " << averageBlockGenIntervalSeconds << "s\n";
    }
...
void
PrintStatsForEachNode(NodeStats* stats, int totalNodes)
{
    for (int i = 0; i < totalNodes; i++)
    {
        ...
        std::cout << "Blue Ratio: "
                  << (stats[i].total_blocks > 0
                          ...
                          : 0)
                  << "%\n";
        std::cout << "Mean Block Propagation: " << stats[i].mean_block_propagation_time << "s\n";
        ...
        if (stats[i].is_miner)
        {
            ...
            std::cout << "Avg Gen Interval: " << stats[i].miner_average_block_gen_interval << "s\n";
        }
    }
}

void
PrintTotalStats(NodeStats* stats,
                int totalNodes,
                double start,
                double finish,
                double averageBlockGenIntervalMinutes)
{
    ...
    double median = propagationTimes.size() > 0 ? propagationTimes[propagationTimes.size() / 2] : 0;
    ...
    std::cout << "Blue Ratio: " << (totalBlocks > 0 ? 100.0 * totalBlueBlocks / totalBlocks : 0)
              << "%\n";
...
void
PrintRegionStats(uint32_t* nodesRegions, uint32_t totalNodes)
{
    ...
    const char* regionNames[] =
        {"North America", "Europe", "South America", "Asia Pacific", "Japan", "Australia", "Other"};

    for (uint32_t i = 0; i < 7; i++)
    {
        if (regions[i] > 0)
        {
            std::cout << "  " << regionNames[i] << ": " << regions[i] * 100.0 / totalNodes << "%\n";
        }
    }
    std::cout << "\n";
}
