 ...
 * Copyright (c) 2026 Eduardo Lechinski Ramos <lechinski@univali.br> revised
 *
...
Ipv4AddressHelperCustom::Ipv4AddressHelperCustom(bool checkAddressDuplication)
...
Ipv4AddressHelperCustom::Ipv4AddressHelperCustom(const Ipv4Address network,
                                                 const Ipv4Mask mask,
...
void Ipv4AddressHelperCustom::SetBase(const Ipv4Address network,
                                      const Ipv4Mask mask,
                                      const Ipv4Address address) {
  NS_LOG_FUNCTION_NOARGS();
...
Ipv4Address Ipv4AddressHelperCustom::NewAddress(void) {
  //
...
Ipv4Address Ipv4AddressHelperCustom::NewNetwork(void) {
  NS_LOG_FUNCTION_NOARGS();
  ++m_network;
  m_address = m_base;
...
Ipv4AddressHelperCustom::Assign(const NetDeviceContainer &c) {
  ...
  for (uint32_t i = 0; i < c.GetN(); ++i) {
    ...
    NS_ASSERT_MSG(
        node,
        "Ipv4AddressHelperCustom::Assign(): NetDevice is not not associated "
        "with any node -> fail");

    Ptr<Ipv4> ipv4 = node->GetObject<Ipv4>();
    NS_ASSERT_MSG(ipv4,
                  "Ipv4AddressHelperCustom::Assign(): NetDevice is associated"
...
uint32_t Ipv4AddressHelperCustom::NumAddressBits(uint32_t maskbits) const {
  NS_LOG_FUNCTION_NOARGS();
