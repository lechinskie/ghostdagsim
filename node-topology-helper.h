...
class GhostDagTopologyHelper
{
  public:
     ...
     * \param latencyParetoShapeDivider Pareto distribution parameter for latency variation
     * \param systemId System identifier for distributed simulations
     */
    GhostDagTopologyHelper(uint32_t noCpus,
     ...
     */
    std::map<uint32_t, std::map<Ipv4Address, double>> GetPeersDownloadSpeeds() const;

    /**
     * \brief Get upload speeds between peers
     * \return Nested map: nodeId -> (peerIp -> uploadSpeed)
     */
    std::map<uint32_t, std::map<Ipv4Address, double>> GetPeersUploadSpeeds() const;
  ...
  private:
    ...
    double m_regionDownloadSpeeds[6]; //!< Average download speed per region (Mbps)
    double m_regionUploadSpeeds[6];   //!< Average upload speed per region (Mbps)

    // Network topology structures
    std::map<uint32_t, std::vector<uint32_t>> m_nodesConnections;       //!< Node connectivity graph
    std::map<uint32_t, std::vector<Ipv4Address>> m_nodesConnectionsIps; //!< IP addresses of peers
    ...
    std::map<uint32_t, std::map<Ipv4Address, double>> m_peersDownloadSpeeds; //!< Download speeds
    std::map<uint32_t, std::map<Ipv4Address, double>> m_peersUploadSpeeds;   //!< Upload speeds
    std::map<uint32_t, NodeInternetSpeeds> m_nodesInternetSpeeds; //!< Node internet speeds
