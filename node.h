...
class BitcoinNode : public Application
{
  ...
  protected:
    ...
    /**
     * \brief Adds the new block in to the blockchain, advertises it to the peers
     * and validates any ophan children
     * \param newBlock the new block
     */
    ...
    /**
     * \brief Advertises the newly validated block when blockTorrent and spv are
     * used
     * \param newBlock the new block
     */
     ...
     */
    void SendMessage(enum Messages receivedMessage,
    ...
    /**
     * \brief Checks if a block has been received but not been validated yet (if
     * it is included in m_receivedNotValidated)
     * \param blockHash the block hash
     * \return true if the block has been received but not validated yet, false
     * otherwise
     */
    ...
    /**
     * \brief Checks if the node has received only the headers of a particular
     * block (if it is included in m_onlyHeadersReceived)
     * \param blockHash the block hash
    ...
    /**
     * \brief Checks if the node has received a particular chunk of a specific
     * block
     * \param blockHash the block hash
    ...
    /**
     * \brief Removes the fist element from m_sendCompressedBlockTimes, when a
     * compressed-block is sent
     */
    void RemoveCompressedBlockSendTime();

    /**
     * \brief Removes the fist element from m_receiveBlockTimes, when a block is
     * received
     */
    void RemoveReceiveTime();

    /**
     * \brief Removes the fist element from m_receiveCompressedBlockTimes, when a
     * compressed-block is received
     */
    ...
    double m_meanBlockReceiveTime;     //!< The mean time interval between two
                                       //!< consecutive blocks (should be around 10min
                                       //!< for bitcoin)
    double m_previousBlockReceiveTime; //!< The time that the node received the
                                       //!< previous block
    double m_meanBlockPropagationTime; //!< The mean time that the node has to wait
    ...
    double m_averageTransactionSize;   //!< The average transaction size. Needed for
                                       //!< compressed blocks
    int m_transactionIndexSize;        //!< The transaction index size in bytes. Needed
                                       //!< for compressed blocks
    bool m_blockTorrent;               //!< True if the blockTorrent mechanism is used, False
                                       //!< otherwise
    uint32_t m_chunkSize;              //!< The size of the chunk in Bytes, when blockTorrent
                                       //!< is used
    bool m_spv; //!< Simplified Payment Verification. Used only in conjuction with
                //!< blockTorrent
    ...
    std::map<std::string, std::vector<Address>>
        m_queueInv; //!< map holding the addresses of nodes which sent an INV for
                    //!< a particular block
    std::map<std::string, std::vector<Address>>
        m_queueChunkPeers; //!< map holding the addresses of nodes from which we
                           //!< are waiting for a CHUNK, key = block_hash
    std::map<std::string, std::vector<int>>
        m_queueChunks; //!< map holding the chunks of the blocks which we have not
                       //!< requested yet, key = block_hash
    std::map<std::string, std::vector<int>>
        m_receivedChunks; //!< map holding the chunks of the blocks which we are
                          //!< currently downloading, key = block_hash
    std::map<std::string, EventId>
        m_invTimeouts; //!< map holding the event timeouts of inv messages
    std::map<std::string, EventId>
        m_chunkTimeouts; //!< map holding the event timeouts of chunk messages
    std::map<Address, std::string> m_bufferedData; //!< map holding the buffered data from previous
                                                   //!< handleRead events
    std::map<std::string, Block> m_receivedNotValidated; //!< vector holding the received but not
                                                         //!< yet validated blocks
    std::map<std::string, Block> m_onlyHeadersReceived;  //!< vector holding the blocks that we know
                                                         //!< but not received
    ...
    std::vector<double> m_sendCompressedBlockTimes; //!< contains the times of the
                                                    //!< next sendBlock events
    std::vector<double> m_receiveBlockTimes; //!< contains the times of the next sendBlock events
    std::vector<double> m_receiveCompressedBlockTimes; //!< contains the times of the next
                                                       //!< sendBlock events
    ...
    const int m_bitcoinMessageHeader;  //!< The size of the bitcoin Message Header,
                                       //!< 90 Bytes, including both the
                                       //!< bitcoinMessageHeaders and the other
                                       //!< protocol headers (TCP, IP, Ethernet)
    const int m_inventorySizeBytes;    //!< The size of inventories in INV messages,
                                       //!< 36 Bytes
